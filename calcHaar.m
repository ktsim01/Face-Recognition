function val = calcHaar(img,haar,pixelX,pixelY,haarX,haarY)
% getCorners() finds the total of the pixel intensity values in a white/black "box"
moveX = haarX-1;
moveY = haarY-1;
val = 0;
if haar == 1 && pixelX+moveX <= size(img,2) && pixelY+moveY<=size(img,1)% top/down white-black
    ...
    val = white-black;
elseif haar == 2 && pixelX+moveX <= size(img,2) && pixelY+moveY<=size(img,1)% left/right white-black
    ...
    val = white-black;
elseif haar == 3 && pixelX+moveX <= size(img,2) && pixelY+moveY<=size(img,1)% top/mid/bottom white-black-white
...
elseif haar == 4  && pixelX+moveX <= size(img,2) && pixelY+moveY<=size(img,1)% left/mid/right white-black-white
    ...
    val = white1 + white2 - black;
elseif haar == 5  && pixelX+moveX <= size(img,2) && pixelY+moveY<=size(img,1)% checkerboard-style white-black-white-black
